...
from .models import TrainingRun, Metric, Checkpoint, Dataset, ModelVersion
...
def load_tabular(path:Path,target:str):
    ext=path.suffix.lower(); df=pd.read_csv(path) if ext=='.csv' else pd.read_json(path,lines=ext=='.jsonl')
    ...
    df=df.drop_duplicates().copy(); y=df.pop(target); X=pd.get_dummies(df,dummy_na=True).replace([np.inf,-np.inf],np.nan)
...
def prepare_splits(X,y,task,seed,ratios):
    tr,va,te=ratios
    if min(ratios)<=0 or abs(sum(ratios)-1)>1e-6: raise ValueError('Split ratios must be > 0 and sum to 1')
    strat=y if task=='classification' and y.nunique()>1 else None; Xtr,Xtmp,ytr,ytmp=train_test_split(X,y,test_size=1-tr,random_state=seed,stratify=strat)
    rel_te=te/(va+te); strat2=ytmp if task=='classification' and ytmp.nunique()>1 else None; Xva,Xte,yva,yte=train_test_split(Xtmp,ytmp,test_size=rel_te,random_state=seed,stratify=strat2)
    scaler=StandardScaler().fit(Xtr); return [(scaler.transform(z).astype('float32'),a.reset_index(drop=True)) for z,a in [(Xtr,ytr),(Xva,yva),(Xte,yte)]],scaler,list(X.columns)
...
def make_targets(ytr,yva,yte,task):
    ...
    labels=sorted(pd.unique(pd.concat([ytr,yva,yte])).tolist(),key=str); mp={v:i for i,v in enumerate(labels)}; return [torch.tensor([mp[v] for v in y],dtype=torch.long) for y in (ytr,yva,yte)],len(labels),labels
...
def evaluate(model,loader,loss_fn,task):
    ...
    if task=='regression':
        pred=np.asarray(ps).reshape(-1); true=np.asarray(ys); return loss,{'mae':float(mean_absolute_error(true,pred)),'rmse':float(mean_squared_error(true,pred)**0.5),'r2':float(r2_score(true,pred))}
    pred=np.argmax(np.asarray(ps),axis=1); p,r,f,_=precision_recall_fscore_support(ys,pred,average='weighted',zero_division=0); return loss,{'accuracy':float(accuracy_score(ys,pred)),'precision':float(p),'recall':float(r),'f1':float(f)}
...
def train_run(db:Session,run_id:str):
    ...
    try:
        ...
        for epoch in range(1,cfg['epochs']+1):
            ...
            tl=total/n; vl,vm=evaluate(model,val_loader,loss_fn,cfg['task']); elapsed=max(time.time()-t0,1e-6); tm=evaluate(model,train_loader,loss_fn,cfg['task'])[1]; metric_key='accuracy' if cfg['task']=='classification' else 'r2'
