...
from .models import TrainingRun, Metric, Checkpoint
...
def load_tabular(path: Path, target: str):
    ...
    elif path.suffix.lower()=='.json': df=pd.read_json(path)
...
def prepare_splits(X, y, task, seed, ratios):
    tr, va, te=ratios
    if abs(tr+va+te-1)>1e-6: raise ValueError('train + validation + test must equal 1')
    ...
    rel_te=te/(va+te)
    strat2=ytmp if task=='classification' and ytmp.nunique()>1 else None
...
def make_targets(ytr,yva,yte,task):
    ...
    labels=sorted(pd.unique(pd.concat([ytr,yva,yte])).tolist()); mp={v:i for i,v in enumerate(labels)}
    return [torch.tensor([mp[v] for v in y],dtype=torch.long) for y in (ytr,yva,yte)], len(labels)

def evaluate(model, loader, loss_fn, task, device):
    model.eval(); losses=[]; ys=[]; ps=[]
    with torch.no_grad():
        for xb,yb in loader:
            out=model(xb.to(device)); loss=loss_fn(out,yb.to(device)); losses.append(loss.item()*len(yb))
            ys.extend(yb.cpu().numpy().tolist()); ps.extend(out.cpu().numpy().tolist())
    ...
    pred=np.argmax(np.asarray(ps),axis=1)
    p,r,f,_=precision_recall_fscore_support(ys,pred,average='weighted',zero_division=0)
...
def train_run(db: Session, run_id: str):
    ...
    try:
        cfg=run.config; set_seed(int(cfg.get('seed',42)))
        workers=int(cfg.get('cpu_workers',1)); workers=max(1,min(CPU_COUNT,workers if workers>0 else CPU_COUNT))
        threads=int(cfg.get('threads',1)); torch.set_num_threads(max(1,threads if threads>0 else CPU_COUNT))
        X,y=load_tabular(Path(db.get(__import__('app.models',fromlist=['Dataset']).Dataset).path) if False else Path(db.get(__import__('app.models',fromlist=['Dataset']).Dataset,run.dataset_id).path),cfg['target'])
        splits=prepare_splits(X,y,cfg['task'],cfg['seed'],(cfg['train_ratio'],cfg['val_ratio'],cfg['test_ratio']))
        (Xtr,ytr),(Xva,yva),(Xte,yte)=splits
        targets,out=make_targets(ytr,yva,yte,cfg['task'])
        model=MLP(Xtr.shape[1],out,cfg['hidden_layers'],cfg['activation'],cfg['dropout'])
        loss_fn=nn.MSELoss() if cfg['task']=='regression' else nn.CrossEntropyLoss()
        opt_cls={'adam':torch.optim.Adam,'sgd':torch.optim.SGD}[cfg['optimizer']]
        ...
        run.status='RUNNING'; db.commit(); run.started_at=__import__('datetime').datetime.utcnow(); db.commit()
        run_dir=settings.storage_root/'runs'/run_id; run_dir.mkdir(parents=True,exist_ok=True)
        best=float('inf')
        for epoch in range(1,cfg['epochs']+1):
            if (run_dir/'cancel').exists(): run.status='CANCELLED'; db.commit(); return
            while (run_dir/'pause').exists():
                run.status='PAUSED'; db.commit(); time.sleep(0.5)
            run.status='RUNNING'; db.commit(); model.train(); t0=time.time(); total=0; n=0
            for xb,yb in train_loader:
                opt.zero_grad(set_to_none=True); outp=model(xb); loss=loss_fn(outp,yb); loss.backward(); opt.step()
                total+=loss.item()*len(yb); n+=len(yb)
            tl=total/n; vl,vm=evaluate(model,val_loader,loss_fn,cfg['task'],'cpu'); elapsed=max(time.time()-t0,1e-6)
            tm=evaluate(model,train_loader,loss_fn,cfg['task'],'cpu')[1]
            db.add(Metric(run_id=run_id,epoch=epoch,split='train',loss=tl,metric=tm.get('accuracy',tm.get('r2')),learning_rate=opt.param_groups[0]['lr'],samples_per_sec=n/elapsed))
            db.add(Metric(run_id=run_id,epoch=epoch,split='validation',loss=vl,metric=vm.get('accuracy',vm.get('r2')),learning_rate=opt.param_groups[0]['lr'],samples_per_sec=n/elapsed))
        ...
        test_loss,test_metrics=evaluate(model,test_loader,loss_fn,cfg['task'],'cpu')
        import json
        (run_dir/'test_metrics.json').write_text(json.dumps({'loss':test_loss,**test_metrics},indent=2))
        run.status='COMPLETED'; run.finished_at=__import__('datetime').datetime.utcnow(); db.commit()
    except Exception as exc:
        run.status='FAILED'; run.error=str(exc); run.finished_at=__import__('datetime').datetime.utcnow(); db.commit()
        raise
