...
from .models import Project, Dataset, Model, TrainingRun, Metric, Checkpoint, Evaluation
from .ml import train_run, load_tabular

Base.metadata.create_all(engine)
app=FastAPI(title='Real ML Training Platform', version='0.1.0')
...
@app.get('/api/health')
def health(): return {'status':'ok','device':'cpu','cpu_cores':CPU_COUNT,'torch_available':True}

@app.get('/api/system/resources')
def resources():
    vm=psutil.virtual_memory(); disk=psutil.disk_usage(str(settings.storage_root));
    return {'cpu_percent':psutil.cpu_percent(interval=0.05),'ram_percent':vm.percent,'ram_bytes':vm.total,'disk_percent':disk.percent,'cpu_cores':CPU_COUNT}
...
def upload_dataset(project_id:str,file:UploadFile=File(...),db:Session=Depends(get_db)):
    ...
    ds=Dataset(project_id=project_id,name=safe_name(file.filename or 'dataset'),format=ext.lstrip('.'),path='')
    db.add(ds); db.flush(); path=settings.storage_root/'datasets'/ds.id; path.mkdir(parents=True,exist_ok=True); fp=path/safe_name(file.filename or 'dataset'); fp.write_bytes(data); ds.path=str(fp)
    try:
        if ext=='.csv': import pandas as pd; df=pd.read_csv(fp)
        else: import pandas as pd; df=pd.read_json(fp,lines=ext=='.jsonl')
...
def validate_dataset(dataset_id:str,target:str,db:Session=Depends(get_db)):
    ...
    try:
        X,y=load_tabular(Path(d.path),target)
        import pandas as pd
        raw=pd.read_csv(d.path) if d.format=='csv' else pd.read_json(d.path,lines=d.format=='jsonl')
...
def create_model(payload:ModelIn,db:Session=Depends(get_db)):
    if payload.task not in {'classification','regression'}: raise HTTPException(400,'Only classification and regression are implemented in this release')
...
def runs(db:Session=Depends(get_db)): return [{'id':r.id,'status':r.status,'error':r.error,'started_at':r.started_at,'finished_at':r.finished_at} for r in db.query(TrainingRun).order_by(TrainingRun.id.desc()).all()]
...
def metrics(run_id:str,db:Session=Depends(get_db)):
    return [{'epoch':m.epoch,'split':m.split,'loss':m.loss,'metric':m.metric,'learning_rate':m.learning_rate,'samples_per_sec':m.samples_per_sec} for m in db.query(Metric).filter(Metric.run_id==run_id).order_by(Metric.epoch,Metric.split).all()]
@app.get('/api/training/runs/{run_id}/checkpoints')
def checkpoints(run_id:str,db:Session=Depends(get_db)):
    return [{'id':c.id,'epoch':c.epoch,'path':c.path,'is_best':c.is_best,'exists':Path(c.path).exists()} for c in db.query(Checkpoint).filter(Checkpoint.run_id==run_id).all()]
@app.get('/api/training/runs/{run_id}/logs')
def logs(run_id:str): return {'run_id':run_id,'message':'Structured application logs are stored by the process supervisor; per-run event streaming is available in the frontend polling endpoint.'}
