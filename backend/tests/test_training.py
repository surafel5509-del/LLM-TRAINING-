...
def test_reproducible_split():
    ...
    a=prepare_splits(X,y,'classification',42,(.7,.15,.15)); b=prepare_splits(X,y,'classification',42,(.7,.15,.15))
    assert np.array_equal(a[0][0],b[0][0])
    targets,n=make_targets(a[0][1],a[1][1],a[2][1],'classification')
    assert n==2 and targets[0].dtype.name=='int64'
